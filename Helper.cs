...
namespace ThiefAndPolice
{
    ...
    {
        public static void AddPersonGrid(Person person, Person[,] matrix, List<Person> personList, int sizeX, int sizeY, int amount) //add to city
        {
            ...
            for (int i = 0; i < amount; i++) // put people in the list
            {
                do
                {
                    posX = rnd.Next(sizeX);
                ...
                } while (xDirection == 0 && yDirection == 0); // check if not both are 0
                ...
                if (person is Police)
                {
                    ...
                    matrix[posX, posY] = police; // add thief to matrix[posX,PosY]
                    personList.Add(police);
                }
                if (person is Citizen)
                {
                    Citizen citizen = new Citizen(posX, posY, xDirection, yDirection);
                    matrix[posX, posY] = citizen; // add thief to matrix[posX,PosY]
        ...
        public static void PoorMovement(Person person, Person[,] poorMatrix, int poorX, int poorY)
        {
            ...
            if (citizen.X >= poorX || citizen.Y >= poorY) //if citizen X,Y is bigger than poor size
            {
                ...
                citizen.X = random.Next(poorX-1);
                citizen.Y = random.Next(poorY-1);
